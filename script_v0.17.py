...
# 3. Замена кириллицы на латиницу в названии папок при назначении клавиш.
# 4. Исправлена ошибка с ссылкой на parent_dir.
...
class FolderFrame(QFrame):
    ...
    def dropEvent(self, event):
        ...
        # Обновлено для доступа к parent_dir
        parent_dir = self.parentWidget().parent_dir

        destination_path = os.path.join(parent_dir, self.folder_name, os.path.basename(source_path))
        try:
...
class PhotoSorter(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Меню "Файл"
        self.file_menu = self.menuBar().addMenu("Файл")  # Изменено для избежания дублирования
        ...
        self.file_menu.addAction(load_dir_action)
        ...
        save_images_action = QAction(QIcon(os.path.join(self.icon_directory, "folder-download.png")), "Сохранить список папок как", self)
        save_images_action.triggered.connect(self.save_images_to_file)
        self.file_menu.addAction(save_images_action)
        ...
        self.update_folder_layout(main_layout)
    ...
    def update_folder_layout(self, main_layout):
    ...
    def add_folder(self):
        ...
        if folder:
            if folder not in self.folders:  # Проверяем, что папка не добавлена дважды
                self.folders.append(folder)  # Сохраняем полный путь к папке
                self.key_map = self.assign_keys()
                self.clear_layout()  # Удаляем старый интерфейс
                self.init_ui()  # Инициализируем новый интерфейс с обновленными папками
                print(f"Добавлена новая папка: {folder}, обновленные ключи: {self.key_map}")
                self.save_folders_to_file()

    def clear_layout(self):
        # Очищаем все элементы из текущего лэйаута
    ...
    def save_folders_to_file(self):
        ...
        with open(sort_dir_list_path, 'w') as file:
            for folder in self.folders:  # Сохраняем только папки
                file.write(f"{folder}\n")
    ...
    def open_folder(self):
        ...
        if new_dir:
            ...
            self.key_map = self.assign_keys()
            self.clear_layout()  # Очищаем текущий интерфейс
            self.init_ui()  # Инициализируем новый интерфейс
    ...
    def load_folders_from_file(self):
        ...
        if file_name:
            with open(file_name, 'r') as file:
                self.folders = [line.strip() for line in file.readlines() if line.strip()]  # Считываем полные пути
            self.key_map = self.assign_keys()
            self.clear_layout()  # Очищаем текущий интерфейс
            self.init_ui()  # Инициализируем новый интерфейс
    ...
    def load_folders_from_directory(self):
        ...
        if dir_path:
            self.folders = [os.path.join(dir_path, f) for f in os.listdir(dir_path) if os.path.isdir(os.path.join(dir_path, f))]
            self.key_map = self.assign_keys()
            self.clear_layout()  # Очищаем текущий интерфейс
            self.init_ui()  # Инициализируем новый интерфейс
            print(f"Список папок загружен из дирекции: {dir_path}")
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key_Left:
            ...
            folder = self.key_map[event.text().lower()]  # Это полный путь к папке
            if modifiers == Qt.ShiftModifier:
                ...
                if self.image_list:
                    ...
                    destination = os.path.join(folder, os.path.basename(source))  # Полный путь к папке
