#!/usr/bin/env python3
# Изменения:
# 1. Исправлено сохранение и загрузка списка папок: теперь используются полные относительные пути относительно self.parent_dir.
# 2. Исправлена обработка пробелов в именах папок при открытии через xdg-open.
# 3. Добавлен пункт "Сохранить список папок как".
# 4. Исправлено динамическое изменение размера иконок папок.
...
from PyQt5.QtGui import (QPixmap, QIcon, QFont, QColor, QPainter, QDrag)
        ...
        if event.button() == Qt.LeftButton:
            ...
            try:
                ...
                if result.returncode == 0:
                    print(f"Папка {folder_path} успешно открыта")
                else:
...
class PhotoSorter(QMainWindow):
    ...
    def init_ui(self):
        ...
        if not self.menuBar().actions():
            ...
            open_action = file_menu.addAction("Открыть папку")
            open_action.triggered.connect(self.open_folder)
            load_file_action = file_menu.addAction("Открыть список папок из файла")
            load_file_action.triggered.connect(self.load_folders_from_file)
            load_dir_action = file_menu.addAction("Открыть список папок из директории")
            load_dir_action.triggered.connect(self.load_folders_from_directory)
            save_action = file_menu.addAction("Сохранить список папок")
            save_action.triggered.connect(self.save_folders_to_file)
            save_as_action = file_menu.addAction("Сохранить список файлов как...")
            save_as_action.triggered.connect(self.save_folders_as)
            clear_action = file_menu.addAction("Очистить список папок")
        ...
        self.create_folder_display(main_layout)

    def create_folder_display(self, main_layout):
    ...
    def create_folder_frame(self, folder):
        ...
        pixmap = QIcon.fromTheme("folder").pixmap(self.width() // 6, self.height() // 6)
    ...
    def save_folders_as(self):
        file_name, _ = QFileDialog.getSaveFileName(self, "Сохранить список папок", "", "Text Files (*.txt)")
        if file_name:
            with open(file_name, 'w') as file:
                for folder in self.folders:
                    file.write(f"{folder}\n")
            print(f"Список папок сохранен как: {file_name}")
    ...
    def resizeEvent(self, event):
        self.init_ui()  # Обновляем интерфейс при изменении размера
