...
# 4. Динамическое изменение размера иконки папки.
...
class FolderFrame(QFrame):
    ...
    def sizeHint(self):
        return QSize(200, 150)  # Измените размер по вашему желанию
...
class PhotoSorter(QMainWindow):
    ...
    def init_ui(self):
        ...
        if not self.menuBar().actions():
            ...
            save_as_action = file_menu.addAction("Сохранить список папок как")
    ...
    def save_folders_as_file(self):
        file_name, _ = QFileDialog.getSaveFileName(self, "Сохранить список папок как", self.current_dir, "Text Files (*.txt)")
        if file_name:
            with open(file_name, 'w') as file:
                for folder in self.folders:
                    file.write(f"{folder}\n")
            print(f"Список папок сохранен в {file_name}")
