...
class PhotoSorter(QMainWindow):
    ...
    def create_folder_frame(self, folder):
        ...
        available_width = min(250, (self.width() - 600) // 4)
        available_height = min(200, (self.height() - 400) // 4)
        frame.setFixedSize(available_width, available_height)
        layout = QVBoxLayout(frame)
        layout.setContentsMargins(0, 0, 0, 0)

        icon_label = QLabel(frame)
        icon_label.setAlignment(Qt.AlignCenter)
        pixmap = QIcon(os.path.join(self.icon_directory, "folder.png")).pixmap(available_width, available_height)
        icon_label.setPixmap(pixmap)
        icon_label.setStyleSheet("QLabel { border-radius: 10px; } QLabel:hover { border: 2px solid #0078d7; }")
        ...
        remove_button.move(available_width - 25, 5)
        ...
        folder_label = QLabel(folder_label_text, frame)
        folder_label.setAlignment(Qt.AlignCenter)
        folder_label.setWordWrap(True)  # Перенос текста
        folder_label.setMaximumWidth(available_width - 10)  # Ограничение ширины
        folder_label.setStyleSheet("""
            font-size: 12px;
            font-weight: bold;
            color: black;
        """)
        folder_label.move((available_width - folder_label.sizeHint().width()) // 2, available_height - folder_label.sizeHint().height() - 5)
    ...
    def update_image(self):
        if self.image_list:
            ...
            if not pixmap.isNull():
                available_width = self.centralWidget().width() - 550
                available_height = self.centralWidget().height() - 450
        ...
        if key == Qt.Key_Left:
            ...
            if modifiers == Qt.ShiftModifier:
                ...
                if self.image_list:
                    ...
                    if self.current_index >= len(self.image_list):
                        self.current_index = len(self.image_list) - 1
                    self.highlight_folder(folder)  # Подсветка папки
                    if self.image_list:
                        self.update_image()
                    else:
                        self.close()
