#!/usr/bin/env python3
# Изменения:
# 1. По умолчанию список папок пустой (self.folders = []).
# 2. В меню "Файл" добавлены:
#    - "Открыть список папок из файла" для загрузки из текстового файла.
#    - "Открыть список папок из директории" для загрузки подпапок из выбранной директории.
# 3. Исправлено открытие папок по клику: возвращено использование xdg-open с полной обработкой ошибок через subprocess.run.
...
class FolderFrame(QFrame):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            folder_path = os.path.join(self.parent().parent_dir, self.folder_name)
            try:
...
class PhotoSorter(QMainWindow):
    def __init__(self):
        ...
        self.folders = []  # По умолчанию пустой список папок
    ...
    def create_folder_frame(self, folder):
        ...
        folder_label_text = f"{folder} ({key})" if key else folder
        ...
        remove_button.setStyleSheet("QPushButton { background-color: red; border-radius: 10px; } QPushButton:hover { background-color: darkred; }")
    ...
    def open_folder(self):
        ...
        if new_dir:
            ...
            self.image_list = self.get_images()
            self.current_index = 0
            self.folders = self.get_folders()  # Оставляем пустой список
            self.key_map = self.assign_keys()
    ...
    def load_folders_from_directory(self):
        ...
        if dir_path:
            self.folders = [f for f in os.listdir(dir_path) if os.path.isdir(os.path.join(dir_path, f))]
            self.key_map = self.assign_keys()
            self.init_ui()
