...
class PhotoSorter(QMainWindow):
    def __init__(self):
        ...
        # Если не указана папка с фотографиями, открываем пустое окно
        if len(sys.argv) > 1:
    ...
    def init_ui(self):
        ...
        # Добавляем кнопку "+" для добавления папки
        add_folder_button_frame = QFrame()
        add_folder_button_frame.setFixedSize(150, 120)
        add_folder_button_frame.setStyleSheet("QFrame { border: 2px dashed #cccccc; border-radius: 10px; }")
        
        add_folder_layout = QVBoxLayout(add_folder_button_frame)
        add_folder_layout.setContentsMargins(5, 5, 5, 5)
        add_folder_layout.setSpacing(5)
        add_folder_layout.setAlignment(Qt.AlignCenter)

        add_button = QPushButton("+")
        add_button.setFixedSize(64, 64)
        ...
        """)
        add_button.clicked.connect(self.add_folder_from_current)
        add_folder_layout.addWidget(add_button, 0, Qt.AlignCenter)

        add_label = QLabel("Добавить папку")
        add_label.setAlignment(Qt.AlignCenter)
        add_label.setWordWrap(True)
        add_label.setMaximumWidth(140)
        add_folder_layout.addWidget(add_label)

        # Добавляем кнопку "+" в начало списка папок
        top_layout.addWidget(add_folder_button_frame)
        ...
        for i, folder in enumerate(self.folders):
            ...
            if i < max_top - 1:  # -1 потому что уже добавили кнопку "+"
                top_layout.addWidget(frame)
            elif i < max_top + max_bottom - 1:
                bottom_layout.addWidget(frame)
            elif i < max_top + max_bottom + max_side - 1:
                left_layout.addWidget(frame)
            elif i < max_top + max_bottom + max_side * 2 - 1:
    ...
    def add_folder_from_current(self):
        """Добавить папку из текущей директории"""
    ...
    def ask_save_folders_before_close(self):
        """Спросить о сохранении списка папок перед закрытием"""
        if self.folders:
            reply = QMessageBox.question(self, "Сохранение списка папок",
                                       "Хотите сохранить список папок перед выходом?",
            ...
            if reply == QMessageBox.Yes:
                ...
                self.close()
            # Если Cancel - остаемся в программе

    def closeEvent(self, event):
        """Обработчик события закрытия окна"""
        if not self.image_list and self.folders:
            self.ask_save_folders_before_close()
            event.ignore()  # Не закрываем окно, ждем решения пользователя
        else:
            event.accept()
    ...
    def delete_image(self):
        if self.image_list:
            ...
            # Если удалили последнее изображение, спрашиваем о сохранении
            if not self.image_list:
                self.ask_save_folders_before_close()
            else:
                self.update_image()
            ...
            if modifiers == Qt.ShiftModifier:
                ...
                if self.image_list:
                    ...
                    try:
                        ...
                        # Если переместили последнее изображение, спрашиваем о сохранении
                        if not self.image_list:
                            self.ask_save_folders_before_close()
                        else:
                            self.update_image()
