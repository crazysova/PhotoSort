...
from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QLabel, QFrame, QVBoxLayout, QHBoxLayout, QWidget, QPushButton, QFileDialog, QMenuBar
)
...
class PhotoSorter(QMainWindow):
    ...
    def get_folders(self):
        ...
        if os.path.exists(sort_dir_list_path):
            ...
            folders = [os.path.join(self.parent_dir, folder) for folder in folders]
        else:
    ...
    def create_folder_frame(self, folder):
        ...
        folder_label = QLabel(f"{os.path.basename(folder)} ({key})", frame)
    ...
    def add_folder(self):
        ...
        if folder:
            folder_name = os.path.relpath(folder, self.parent_dir)  # Сохраняем относительный путь
