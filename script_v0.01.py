...
class PhotoSorter(QMainWindow):
    def __init__(self):
        ...
        self.active_folder = None  # Для отслеживания текущей активной папки
    ...
    def create_folder_frame(self, folder):
        """Создаем рамку для папки с иконкой, названием и клавишей."""
        frame = QFrame()
        frame.setFrameShape(QFrame.Box)
        frame.setStyleSheet("QFrame { border: 1px solid black; } QFrame:hover { border: 2px solid blue; }")  # Подсветка при наведении
        layout = QHBoxLayout(frame)

        # Добавляем иконку папки
        icon_label = QLabel()
        icon_label.setPixmap(QIcon.fromTheme("folder").pixmap(16, 16))  # Системная иконка папки
        ...
        # Сохраняем ссылку на frame для изменения цвета позже
        frame.folder_name = folder
        frame.setMouseTracking(True)  # Включаем отслеживание мыши для hover
        return frame
    ...
    def update_folder_colors(self):
        """Обновляем цвета рамок папок на основе активной папки."""
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key_Left:
            ...
            if modifiers == Qt.ShiftModifier:
                ...
                if self.image_list:
                    self.active_folder = folder  # Устанавливаем активную папку
                    self.update_folder_colors()  # Обновляем цвета
